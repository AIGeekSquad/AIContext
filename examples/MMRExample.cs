...
public class MMRExample
{
    public static void RunExample()
    {
        ...
        var query = Vector<double>.Build.DenseOfArray([
    ...
    private static List<Vector<double>> CreateSampleDocuments()
    {
        ...
        return
        [
            Vector<double>.Build.DenseOfArray([0.8, 0.2, 0.0, 0.0, 0.0]), // ML intro
            Vector<double>.Build.DenseOfArray([0.85, 0.15, 0.0, 0.0, 0.0]), // Advanced ML (similar)

            // Sports cluster  
            Vector<double>.Build.DenseOfArray([0.1, 0.8, 0.1, 0.0, 0.0]), // Basketball
            Vector<double>.Build.DenseOfArray([0.0, 0.9, 0.1, 0.0, 0.0]), // Football (similar)

            // Cooking cluster
            Vector<double>.Build.DenseOfArray([0.0, 0.1, 0.8, 0.1, 0.0]), // Italian cooking
            Vector<double>.Build.DenseOfArray([0.0, 0.0, 0.9, 0.1, 0.0]), // French cooking (similar)

            // Environment cluster
            Vector<double>.Build.DenseOfArray([0.0, 0.0, 0.1, 0.8, 0.1]), // Climate change
            Vector<double>.Build.DenseOfArray([0.0, 0.0, 0.0, 0.9, 0.1]), // Conservation (similar)

            // Finance cluster
            Vector<double>.Build.DenseOfArray([0.1, 0.0, 0.0, 0.1, 0.8]), // Stock market
            Vector<double>.Build.DenseOfArray([0.0, 0.0, 0.0, 0.1, 0.9])
        ];
    }
    ...
    public static void RunExample()
    {
        ...
        var contextCandidates = new List<(string text, Vector<double> embedding)>
        {
            ("Machine learning is a subset of artificial intelligence.", 
             Vector<double>.Build.DenseOfArray([0.9, 0.1, 0.0])),
            ("Deep learning uses neural networks with multiple layers.", 
             Vector<double>.Build.DenseOfArray([0.85, 0.15, 0.0])),
            ("Artificial intelligence can solve complex problems.", 
             Vector<double>.Build.DenseOfArray([0.8, 0.2, 0.0])),
            ("Natural language processing helps computers understand text.", 
             Vector<double>.Build.DenseOfArray([0.7, 0.1, 0.2])),
            ("Computer vision enables machines to interpret images.", 
             Vector<double>.Build.DenseOfArray([0.6, 0.1, 0.3])),
            ("Robotics combines AI with mechanical engineering.", 
             Vector<double>.Build.DenseOfArray([0.4, 0.2, 0.4])),
            ("Data science involves extracting insights from data.", 
             Vector<double>.Build.DenseOfArray([0.3, 0.4, 0.3])),
            ("Statistics provides foundation for machine learning.", 
             Vector<double>.Build.DenseOfArray([0.5, 0.3, 0.2]))
        };
        
        // User question about AI applications
        var queryEmbedding = Vector<double>.Build.DenseOfArray([0.8, 0.1, 0.1]);
